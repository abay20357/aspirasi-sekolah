...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-fade-in-up delay-100"
            x-data="{ role: '{{ old('role', $user->role) }}', isSubmitting: false }">
            <div class="p-6 md:p-8">
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST" class="space-y-5"
                    ...
                    <!-- Password (Shared) -->
                    <div>
                        <label for="password" class="block text-sm font-semibold text-slate-700 mb-1.5">Password <span
                                class="text-xs text-slate-400 font-normal">(kosongkan jika tidak diubah)</span></label>
